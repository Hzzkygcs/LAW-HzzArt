...
COMPOSE_ALL_FILES := -f docker-compose.yml -f docker-compose.hzzart.yml -f docker-compose.monitor.yml -f docker-compose.tools.yml -f docker-compose.nodes.yml -f docker-compose.logs.yml
COMPOSE_MONITORING := -f docker-compose.yml -f docker-compose.hzzart.yml -f docker-compose.monitor.yml
COMPOSE_LOGGING := -f docker-compose.yml -f docker-compose.hzzart.yml -f docker-compose.logs.yml
COMPOSE_TOOLS := -f docker-compose.yml -f docker-compose.hzzart.yml -f docker-compose.tools.yml
COMPOSE_NODES := -f docker-compose.yml -f docker-compose.hzzart.yml -f docker-compose.nodes.yml
HZZART_SERVICES := mongodb postgres authentication-service admin-service login-orchestration frontend-service nginx art-collection-service collection-interactions-orchestration export-collection-orchestrations like-comment-service video-processing-service video-processing-service-2
...
ELK_MAIN_SERVICES := ${ELK_SERVICES} ${ELK_MONITORING} ${ELK_TOOLS} ${HZZART_SERVICES}
ELK_ALL_SERVICES := ${ELK_MAIN_SERVICES} ${ELK_NODES} ${ELK_LOG_COLLECTION} ${HZZART_SERVICES}
